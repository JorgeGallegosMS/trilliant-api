...
module.exports = {
    ...
    getReviewById: async(id) => {
        try {
            const review = Reviews.findOne({_id: id});
            if (!review) {
                throw new CustomError({
    ...
    uploadImage: async(id, url, filename) => {
        try {
            const fields = {
                userId: id,
                url: url,
    ...
    rateCommentReview: async(reviewId, data) => {
        try {
            const reviewToUpdate = await Reviews.findOneAndUpdate({_id: reviewId}, {$set: {overall: data.overall, 
                quality: data.quality, fit: data.fit, shipping: data.shipping, comment: data.comment, shouldDisplay: true}});
    ...
    getRates: async(reviewsList) => {
        try {
            const sum = await Reviews.aggregate([
                {$match: {_id: {$in: reviewsList}, shouldDisplay: true}}
            ]);
            const overallSum = sum.map((item) => {
                return item.overall
            }).reduce((prev, next) => {
                return +prev + +next
            }) / sum.length;
            const qualitySum = sum.map((item) => {
                return item.quality
            }).reduce((prev, next) => {
                return +prev + +next
            }) / sum.length;
            const fitSum = sum.map((item) => {
                return item.fit
            }).reduce((prev, next) => {
                return +prev + +next
            }) / sum.length;
            const shippingSum = sum.map((item) => {
	...
	updateLooksGreatValue: async(action, userId, id) => {
		try {
			if (action === 'add') {
                ...
                const review = await Reviews.findOneAndUpdate({_id: id}, { $addToSet: { looksGreat: userId}}, {new: true});
