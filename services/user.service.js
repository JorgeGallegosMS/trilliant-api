...
module.exports = {
	...
	removeDeletedReviewStatsFromUserModel: async (userId, deletedReviewStats) => {
		try {
			const userWithRemovedStats = await User.findByIdAndUpdate(userId, {$inc: {looksGreatCount: -deletedReviewStats.looksGreatCount, 
				helpfulCount: -deletedReviewStats.helpfulCount}});
				return;
		} catch(err) {
			throw new CustomError({
				message: err.message,
